...
- **Randomness**: Employs **LFSR (linear feedback shift register) based pseudo-random number generation** for randomness.
...
- **AI Solver**: Uses **Q-learning** and **[SARSA](#sarsa-algorithm-for-maze-pathfinding) with [UCB1](#ucb1-exploration-for-maze-pathfinding) exploration** for pathfinding.
...
- **AI Integration**: Employs **SARSA with UCB1 exploration** in `pynq_rle.py`, handling **256 states** (16x16 grid), **4 actions** (directions), and rewards, trained on **10,000+ mazes**.
- **HW-SW Co-Design**: FPGA accelerates maze generation (**6-8x faster** than CPU in `maze_generator_pc.py`), while the ARM CPU manages RLE solving and Pygame visualisation in real-time (**<40ms solves**).
- **Complexity Metrics**: 
...
- **Unique Value**: FPGA + RLE offers **2.22x end-to-end speedup**, perfect for robotics and gaming.
